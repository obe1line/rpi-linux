...
/*
 * Driver for the Innodisk EVDM-OOM1 camera module.
 *
 * Copyright (C) 2024, Chris Storah <christopher.storah@leicabiosystems.com>
 *
 */
...
#define AP1302_VIDEO_WIDTH			AP1302_REG_16BIT(0x4000)
#define AP1302_VIDEO_HEIGHT			AP1302_REG_16BIT(0x4002)
...
/* System Registers */
#define AP1302_DBG_CTRL				AP1302_REG_16BIT(0x6000)
...
static int ap1302_dump_console(struct ap1302_device *ap1302)
{
    ...
    dev_info(ap1302->dev, "Start Console buffer\n");
    ...
    dev_info(ap1302->dev, "End Console Buffer\n");
...
static int ap1302_set_hblank(struct ap1302_device *ap1302, s32 hblank)
{
	dev_dbg(ap1302->dev,"Set Hblank %d\n", hblank);
	return 0;  
}

static int ap1302_get_vblank(struct ap1302_device *ap1302, s32 *value)
{
	dev_dbg(ap1302->dev,"Get Vblank\n");
	*value = 0;
	return 0;  
}

static int ap1302_set_vblank(struct ap1302_device *ap1302, s32 value)
{
	dev_dbg(ap1302->dev,"Set Vblank %d\n", value);
	return 0;  
}
...
static int ap1302_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_HBLANK:
		return ap1302_set_hblank(ap1302, ctrl->val);

	case V4L2_CID_VBLANK:
		return ap1302_set_vblank(ap1302, ctrl->val);
...
static int ap1302_g_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		return ap1302_get_vblank(ap1302, &ctrl->val);
...
#define HZ_TO_S15_16_MHZ(hz) \
	...
	ret = ap1302_poll_timeout(ap1302, AP1302_BOOTDATA_STAGE,
			value, value==0xFFFF, 20000, 10000000);
	if (ret < 0)
	{
		dev_err(ap1302->dev,
...
static void dump_regs(struct ap1302_device *ap1302)
{
	unsigned int value;
	int ret;

	ret = ap1302_read(ap1302, AP1302_CHIP_VERSION, &value);
   dev_info(ap1302->dev, "AP1302_CHIP_VERSION 0x%04X\n", value);
   ret = ap1302_read(ap1302, AP1302_CHIP_REV, &value);
   dev_info(ap1302->dev, "AP1302_CHIP_REV 0x%04X\n", value);
   ret = ap1302_read(ap1302, AP1302_VIDEO_WIDTH, &value);
   dev_info(ap1302->dev, "AP1302_VIDEO_WIDTH 0x%04X (%d)\n", value, value);
   ret = ap1302_read(ap1302, AP1302_VIDEO_HEIGHT, &value);
   dev_info(ap1302->dev, "AP1302_VIDEO_HEIGHT 0x%04X (%d)\n", value, value);

	unsigned int i;
	for (i = 0x4000; i < 0x4030; i+=2) {
		u32 val;

		ret = ap1302_read(ap1302, AP1302_REG_16BIT(i), &val);
		if (ret)
			return;

		dev_info(ap1302->dev, "R0x%04x = 0x%08x\n", i, val);
	}
}
...
static int ap1302_detect_chip(struct ap1302_device *ap1302)
{
	...
	if (version != AP1302_CHIP_ID) {
        ...
        dev_info(ap1302->dev, "** Note: chip version should be correct if the sensors are powered **");
//		return -EINVAL;
	}
...
static int ap1302_hw_init(struct ap1302_device *ap1302)
{
	...
	if (ret < 0)
		goto error_firmware;
   ap1302_write(ap1302, AP1302_DBG_CTRL, 0xFFFF, &ret);  // Full debug
	/*
...
MODULE_AUTHOR("Chris Storah <christopher.storah@leicabiosystems.com>");

MODULE_DESCRIPTION("Innodisk EVDM-OOM1 camera module driver");
MODULE_LICENSE("GPL");
